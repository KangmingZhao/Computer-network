...
int receivemessage() {
    ...
    while (true) {
        ...
        while (true) {
            ...
            //if (header.seq==expectedSeq&&header.flag == OVER && cal_ck_sum((u_short*)&recvbuffer, sizeof(header)+header.len) == 0) {
            //    if (endreceive()) {
            //        return 1; // 结束消息接收
            //    }
            //    cout << "传输结束" << endl;
         ...
         /*   else if (header.flag == OVER) {
                ...
                cout << cal_ck_sum((u_short*)&recvbuffer, sizeof(header)) << endl;
                cout << "数据包出错，正在等待重传" << endl;
                continue;
            }*/

            if (header.seq == expectedSeq && cal_ck_sum((u_short*)recvbuffer, sizeof(header) +header.len) == 0) {
        ...
        expectedSeq = expectedSeq + 1; // 更新期望的序列号
    }
}
